...
## temporary code 4.5 (20190506) version=v1.4
...
parser = OptionParser(usage=usage, version="v1.4")
...
parser.add_option("-c","--cutoff",action="store",type="float",dest="cutoff",default=0.1,help="threshold for the presence of each sequence [Default: 0.1] times as compared with the average read depth calculated from Mtb genome size")
...
cutoff = options.cutoff                 # threshold for the presence of each unit; xx times as compared with the average read depth 
    ...
    def parse_blast_log2(self,input_file, in_file,log2_handle):
        ...
        def extract_seq(Input_file, Subject, SStart, SEnd):
            ...
            if (int(SStart)>int(SEnd)):
                ...
                rev_comp="-i"
            try:
                cmd="samtools faidx "+Input_file+" "+Subject+":"+SStart+"-"+SEnd+" -n 200 -c "+rev_comp
...
if __name__ == "__main__":
    ...
    if not seq:
        ...
        if swift == 'on':
            ...
            min_strict = max(1,int(setlength * cutoff / 4500000))
            ...
            if (narg == 2) and (remaining > 0):
                ...
                if out_second < remaining:
                    min_strict = max(1,int((out_first + out_second) * cutoff / 4500000))
            elif (narg == 1) and (remaining > 0):
                    min_strict = max(1,int(out_first * cutoff / 4500000))
