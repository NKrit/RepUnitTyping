...
swift = options.swift                   # swift mode, default: on
...
class Main:
    ...
    def parse_blast(self,in_file,qlabels,log_handle,out_handle):  # Parse blast output file
        ...
        for line in file_blast:
            ...
            if not re.search('#',line):
                ...
                tmp[12]=int(tmp[12])
                # for min_strict (=record) and min_relax (=record_relax)
                # if query length is fixed to 25 bp, 24 of 25 bp match indicates just 96%
                # with the repeat unit, however, it is not the case 
                # here thres = 100*(float(tmp[12])-1.1)/float(tmp[12]) is a better criterion (minus 2 is too much) 
        ...
        for i in range(query_n):
            ...
            # min_relax Default: 0.12 * average read depth
            #if (record[qlabels[i]] >= min_strict) or (record_relax[qlabels[i]]) >= min_relax:
    ...
    def parse_blast_pred(self,in_file,qlabels,log_handle,out_handle):  # Parse blast output file for prediction of the  number of repeat units
        ...
        for line in file_blast:
            ...
            if not re.search('#',line):
                ...
                # if query length is fixed to 25 bp, 24 of 25 bp match indicates just 96%
                # with the repeat unit, however, it is not the case 
                # here thres = 100*(float(tmp[12])-1)/float(tmp[12]) is a better criterion (minus 2 is too much) 
                thres = 100*(float(tmp[12])-1)/float(tmp[12])
                #print("\nthres: ")
                #print(thres)
                if tmp[2]==100 and tmp[3]==tmp[12]:
        ...
        for i in range(query_n):
            ...
            # min_strict Default: 0.1 * average read depth
                ...
                RepeatHits[qlabels_sep[i][0]] += float(record[qlabels[i]])

        log_handle.write("## %s" % "[PREDICTION]")
        log_handle.write("\n## %s\t%s\t%s\t%s\n" % ("RepLocus","NoRepeats","NoFL","CV>0.2_unstableFL"))

        for qgroup in RepeatHits:
            FLnum=FLnums[qgroup]
            if FLnum>0:
                FLmean = FlankingHits[qgroup]/FLnum
                signal_unit = RepeatHits[qgroup]/FLmean
                if FLnum>1:
                    # SD = ([sum of squares divided by N] minus [square of mean])^0.5
                    coeff_var = (((FlankingSqs[qgroup]/FLnum)-(FLmean**2))**0.5)/FLmean
                    storage.append([round(signal_unit,1),round(FLmean,1),round(coeff_var,1)])
                    log_handle.write("%s\t%3.1f\t%d\t%3.1f\n" % (qgroup,signal_unit,FLnum,coeff_var))
                else:
                    storage.append([round(signal_unit,1),round(FLmean,1),"NA"])
                    log_handle.write("%s\t%3.1f\t%d\t%s\n" % (qgroup,signal_unit,FLnum,"NA"))
            else:
    ...
    def parse_blast_log2(self,in_file,log2_handle):
        #Using ordered dictionary
        knownqs = []
        unknownqs = []
        # 13th column (non-standard option)
        ...
        for line in file_blast:
            ...
            if not re.search('#',line):
                ...
                thres = 100*(float(tmp[12])-1)/float(tmp[12]) 
                if (tmp[2]==100 and tmp[3]==tmp[12]):
                    knownqs.append([tmp[0],tmp[1],tmp[8],tmp[9],tmp[2],round(100.0,1),"Listed"])
                elif (tmp[2]>=thres and tmp[3]>=tmp[12]-1):
                    tmp[0]= str(tmp[0].split("_")[0])
                    matched_len=round(float(tmp[3])*100/float(tmp[12]),1)
                    unknownqs.append([tmp[0],tmp[1],tmp[8],tmp[9],tmp[2],matched_len,"Unlisted"])
        for unknownq in unknownqs:
            for knownq in knownqs: 
                if knownq[1]==unknownq[1]:
                    if (knownq[2]==unknownq[2] or knownq[3]==unknownq[3]):
                        unknownq[6]="Duplicated"
        print("\nListed hits: "),
        print(sum(knownq[6].count("Listed") for knownq in knownqs))
        print("Dupicated hits: "),
        print(sum(unknownq[6].count("Duplicated") for unknownq in unknownqs))
        print("Possibly unlisted hits: "),
        print(sum(unknownq[6].count("Unlisted") for unknownq in unknownqs))
...
if __name__ == "__main__":
    ...
    if not seq:
        ...
        if swift == 'on':
            ...
            if (narg == 2) and (remaining > 0):
                ...
                if out_second < remaining:
                    ...
                    min_relax = max(2,int((out_first + out_second) * 0.12 / 4500000))
            elif (narg == 1) and (remaining > 0):
                    min_strict = max(1,int(out_first * 0.1 / 4500000))
                    min_relax = max(2,int(out_first * 0.12 / 4500000))
            else:
                    min_strict = max(1,int(setlength*0.1 / 4500000))
                    min_relax = max(2,int(setlength*0.12 / 4500000))
        else:
        ...
        print("[CAUTION]")
        print("Fastq read data containing a PCR-amplification step cannot predict repeat numbers correctly!!\n")
    ...
    log.write("## ver=%s | min(acceptable error-free hits)=%d | rmin(acceptable one error hits)=%d | pred=%s | seq=%s | swift=%s | filter=%s| sorted=%s\n" %(ver, min_strict, min_relax, pred, seq, swift, filt, sortS))
    ...
    if seq:
        log2.write("## Query\tSubject\tSStart\tSEnd\t%identity\tAlign_Query_R\tNote\n")
        t.parse_blast_log2("%s.blast.out" % tmpfile,log2)
    else:
